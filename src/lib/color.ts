...
export function oklchToRgb({ l, c, h }: OKLCH): RGB {
  ...
  const sv = l - 0.0894841775 * a - 1.2914855480 * b;
  const rl = lv * lv * lv, ml = mv * mv * mv, sl = sv * sv * sv;
  const rLin = +4.0767416621 * rl - 3.3077115913 * ml + 0.2309699292 * sl;
  const gLin = -1.2684380046 * rl + 2.6097574011 * ml - 0.3413193965 * sl;
  const bLin = -0.0041960863 * rl - 0.7034186147 * ml + 1.7076147010 * sl;
  return {
...
export function oklchToHsl({ l, c, h }: OKLCH): HSL {
  ...
  let hv = max === rn ? (gn - bn) / d + (gn < bn ? 6 : 0)
    : max === gn ? (bn - rn) / d + 2
    : (rn - gn) / d + 4;
